...
void freeVario_RTOS_Init() {
    ...
    osThreadDef(sendDataTask, StartSendDataTask, osPriorityAboveNormal, 0,
            2048);
...
void StartDefaultTask(void const * argument) {
    ...
    for (;;) {
        ...
        if (UserPowerButton) {
            ...
            if (((xTaskGetTickCount() - pwrBTtimePressed) > PWRBUTTONDELAY)
        ...
        if (UserOkButton) {
            ...
            if (((xTaskGetTickCount() - OkButtonTimePressed) > 2000)
        ...
        if (UserCancelButton) {
            ...
            if (((xTaskGetTickCount() - CancelButtonTimePressed) > 4000)
        ...
        if (UserNextButton) {
            ...
            if (((xTaskGetTickCount() - NextButtonTimePressed) > 2000)
                    & (HAL_GPIO_ReadPin(BTN_NEXT_GPIO_Port, BTN_NEXT_Pin)
                            == GPIO_PIN_SET)) {
                UserNextButton = 0;
        ...
        if (UserPrevButton) {
            osDelay(20);
            if (((xTaskGetTickCount() - PrevButtonTimePressed) > 2000)
                    & (HAL_GPIO_ReadPin(BTN_PREV_GPIO_Port, BTN_PREV_Pin)
                            == GPIO_PIN_SET)) {
                UserPrevButton = 0;
                activity.useKalman = (activity.useKalman == 0) ? 1 : 0;
            }

            if (HAL_GPIO_ReadPin(BTN_PREV_GPIO_Port, BTN_PREV_Pin)
                    == GPIO_PIN_RESET) {
                UserPrevButton = 0;
            }
        }
        ...
        if (maintenanceloop > 600) { // = 1 minute
            ...
            if (hgps.fix > 0 && hgps.is_valid && hgps.sats_in_use > 6) { //have accurate fix
                activity.barognssdeveation = sensors.AltitudeMeters
                        - (hgps.altitude * 1000);
