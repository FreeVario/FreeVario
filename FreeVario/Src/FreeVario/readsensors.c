...
char _orientation[9] = { 0, -1, 0, 1, 0, 0, 0, 0, -1 };
...
void setupReadSensorsMPU6050(SD_MPU6050 *mpu1) {
    SD_MPU6050_Init(&FV_I2C, mpu1, FV_ACCL_ADR, SD_MPU6050_Accelerometer_4G,
...
void setupKalman() {
    ...
    KalmanFilter_Configure(Z_VARIANCE, ZACCEL_VARIANCE, ZACCELBIAS_VARIANCE, 0,
            0.0f, 0.0f);
...
void calcSensorsKalman(BMP280_HandleTypedef *bmp280, SD_MPU6050 *mpu1) {
    ...
    sensors.gyro_x = (ACCLSMOOTH * sensors.gyro_x + mpu1->Gyroscope_X)
            / (ACCLSMOOTH + 1);
    sensors.gyro_y = (ACCLSMOOTH * sensors.gyro_y + mpu1->Gyroscope_Y)
            / (ACCLSMOOTH + 1);
    sensors.gyro_z = (ACCLSMOOTH * sensors.gyro_z + mpu1->Gyroscope_Z)
            / (ACCLSMOOTH + 1);
    ...
    float accelv = 9.8f
            * sqrtf(powf(sensors.accel_x, 2) + powf(sensors.accel_z, 2)); //just the z and x axes

    sensors.gforce = accelv / 100;

    if (activity.useKalman) {
        KalmanFilter_Update((float) sensors.VarioMsRaw / 10, accelv / 100,
                (float) SENSORREADMS / 1000, &zTrack, &vTrack); // values must be cm/s
...
float getAltitudeMeters() {
    ...
    return 44330.0f
            * (1.0f
                    - powf(
                            ((float) sensors.pressure / 100)
                                    / (float) conf.qnePressure, 0.1902949f)); //because Pressure is in Pa x 100
...
void calculateVario50ms() {
    ...
    if (SO_qisFull(&sensors.QAltitudeMeters)) {
        ...
        sensors.VarioMsRaw = (int32_t) ((SO_rear(&sensors.QAltitudeMeters)
                - SO_front(&sensors.QAltitudeMeters)) * 1000) * 2;
